bart_mediate1 <- function(data_train, data_test, formula_m, formula_y,
  ...
  for (j in 1:chains) {
    ...
    hypers_tau             <- Hypers(X_m_train, m_scale, gamma = gamma, num_tree = num_tree, normalize_Y = FALSE)
    hypers_mu_m            <- Hypers(X_m_train, m_scale, normalize_Y = FALSE)
    opts_mu_m              <- Opts(update_s = FALSE, update_sigma_mu = FALSE)
    opts_tau               <- opts_mu_m
    opts_tau$update_sigma  <- FALSE
    
    ## Hypers for mu_y, zeta, d ----
    hypers_mu_y            <- Hypers(X_y_train, Y_scale, gamma = gamma, num_tree = num_tree, normalize_Y = FALSE)
    hypers_zeta            <- Hypers(X_y_train, Y_scale, gamma = gamma, num_tree = num_tree, normalize_Y = FALSE)
    hypers_d               <- Hypers(X_y_train, Y_scale, normalize_Y = FALSE)
  ...
  return(list(
    forest_mu_m = forest_mu_m,
    forest_tau = forest_tau,
    forest_mu_y = forest_mu_y,
    forest_zeta = forest_zeta,
    forest_d = forest_d,
...
bart_mediate2 <- function(data_train, data_test, formula_m, formula_y,
  ...
  for (j in 1:chains) {
    ...
    ## Hypers for mu_m and tau ----
    hypers_tau             <- Hypers(X_m_train, m_scale, gamma = gamma, num_tree = num_tree, k = k, normalize_Y = FALSE)
    hypers_mu_m            <- Hypers(X_m_train, m_scale, normalize_Y = FALSE)
    opts_mu_m              <- Opts(update_s = FALSE, update_sigma_mu = FALSE)
    opts_tau               <- opts_mu_m
    opts_tau$update_sigma  <- FALSE
    ...
    ## Hypers for mu_y, zeta, d ----
    hypers_mu_y            <- Hypers(X_y_train, Y_scale, gamma = gamma, num_tree = num_tree, k = k, normalize_Y = FALSE)
    hypers_zeta            <- Hypers(X_y_train, Y_scale, gamma = gamma, num_tree = num_tree, k = k, normalize_Y = FALSE)
    hypers_d               <- Hypers(X_y_train, Y_scale, normalize_Y = FALSE)
    opts_mu_y              <- Opts(update_s = FALSE, update_sigma_mu = FALSE)
    opts_zeta              <- opts_d <- opts_mu_y
    opts_zeta$update_sigma <- FALSE
...
bart_mediate3 <- function(data_train, data_test, formula_m, formula_y,
                          pi_hat_train, pi_hat_test,
                          m0_hat_train, m0_hat_test,
                          m1_hat_train, m1_hat_test,
                          mediator_name, outcome_name, treat_name,
  ...
  for (j in 1:chains) {
    ...
    hypers_gamma           <- hypers_mu_y
    opts_mu_y              <- Opts(update_s = FALSE, update_sigma_mu = FALSE)
    opts_gamma <- opts_zeta  <- opts_d <- opts_mu_y
    opts_zeta$update_sigma <- FALSE
    opts_d$update_sigma    <- FALSE
    opts_gamma$update_sigma <- FALSE
    ...
    forest_zeta <- MakeForest(hypers_zeta, opts_zeta)
    forest_d    <- MakeForest(hypers_d, opts_d)
    forest_gamma <- MakeForest(hypers_gamma, opts_gamma)
    ...
    zeta <- forest_zeta$do_predict(X_y_train)
    d <- forest_d$do_predict(X_y_train)
    gamma <- forest_gamma$do_predict(X_y_train)
    
    # Store samples for one chain
    mu_y_samples <- matrix(NA, nrow = n_iter - burnin, ncol = nrow(X_y_test))
    zeta_samples <- matrix(NA, nrow = n_iter - burnin, ncol = nrow(X_y_test))
    d_samples <- matrix(NA, nrow = n_iter - burnin, ncol = nrow(X_y_test))
    gamma_samples <- matrix(NA, nrow = n_iter - burnin, ncol = nrow(X_y_test))
    mu_m_samples <- matrix(NA, nrow = n_iter - burnin, ncol = nrow(X_y_test))
    tau_samples <- matrix(NA, nrow = n_iter - burnin, ncol = nrow(X_y_test))
    sigma_y_samples <- rep(NA, n_iter - burnin)
    sigma_m_samples <- rep(NA, n_iter - burnin)
    ...
    for (i in 1:n_iter) {
      ...
      mu_y  <- forest_mu_y$do_gibbs(X_mu_y_train, Y_scale - A * zeta - m_scale * (d + A * gamma), X_mu_y_train, 1)
      mu_y_test <- forest_mu_y$do_predict(X_mu_y_test)
      sigma_y <- forest_mu_y$get_sigma()
      ...
      zeta  <- forest_zeta$do_gibbs(
        X_y_train[A == 1,], 
        Y_scale[A == 1] - mu_y[A == 1] - (m_scale * (d + gamma))[A == 1], 
        X_y_train, 
        1)
      zeta_test <- forest_zeta$do_predict(X_y_test)
      
      R <- (Y_scale - mu_y - A * zeta - c(m_scale * A * gamma)) / m_scale
      forest_d$set_sigma(sigma_y)
      d <- forest_d$do_gibbs_weighted(
        X_y_train, 
        R, 
      ...
      Rg <- (Y_scale[A == 1] - mu_y[A == 1] - (A * zeta + m_scale * d)[A == 1]) / m_scale[A == 1]
      forest_gamma$set_sigma(sigma_y)
      gamma <- forest_d$do_gibbs_weighted(
        X_y_train[A == 1,], 
        Rg, 
        m_scale[A == 1]^2, 
        X_y_train, 
        1)
      gamma_test <- forest_gamma$do_predict(X_y_test)
      
      # Unscale
      mu_y_unscale <- mean(Y) + (mu_y_test * sd(Y)) - mean(m) / sd(m) * (d_test + A * gamma_test) * sd(Y)
      zeta_unscale <- zeta_test * sd(Y)
      d_unscale <- d_test * sd(Y) / sd(m)
      gamma_unscale <- gamma_test * sd(Y) / sd(m)
      mu_m_unscale <- mean(m) + (mu_m_test * sd(m))
      tau_unscale <- tau_test * sd(m)
      sigma_y_unscale <- sigma_y * sd(Y)
      sigma_m_unscale <- sigma_m * sd(m)
      ...
      if (i > burnin){
        ...
        zeta_samples[i - burnin,] <- zeta_unscale
        d_samples[i - burnin,] <- d_unscale
        gamma_samples[i - burnin,] <- gamma_unscale
        mu_m_samples[i - burnin,] <- mu_m_unscale
        tau_samples[i - burnin,] <- tau_unscale
        sigma_y_samples[i - burnin] <- sigma_y_unscale
        sigma_m_samples[i - burnin] <- sigma_m_unscale
  ...
  return(list(
    ...
    zeta_samples = do.call(rbind, zeta_list),
    d_samples = do.call(rbind, d_list),
    gamma_samples = do.call(rbind, gamma_list),
    mu_m_samples = do.call(rbind, mu_m_list),
    tau_samples = do.call(rbind, tau_list),
    sigma_y_samples = unlist(sigma_y_list),
    sigma_m_samples = unlist(sigma_m_list), 
